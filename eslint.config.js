...
export default defineConfig([
  ...
  // Block B: src/components/ui/ must be pure — no API or React Query imports
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        patterns: [
          { group: ['*/api/*', '@/api/*'], message: 'Pure UI components cannot import from api/. Move logic to a feature hook or shared component.' },
          { group: ['@tanstack/react-query'], message: 'Pure UI components cannot use React Query. Move logic to a feature hook.' },
        ],
      }],
    },
  },
