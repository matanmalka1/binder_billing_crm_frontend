...
export const TaxUpcomingDeadlines = ({ items }: TaxUpcomingDeadlinesProps) => {
  const subtitle = items.length > 0 ? `${items.length} מועדים ב-7 הימים הקרובים` : undefined;

  return (
    <Card variant="elevated" title="מועדים קרובים" subtitle={subtitle}>
      {items.length === 0 ? (
        <EmptyState icon={CalendarClock} message="אין מועדים קרובים השבוע" variant="minimal" />
      ) : (
            ...
            return (
              <li
                ...
                className={cn(
                  ...
                  isUrgent ? "border-orange-200 bg-orange-50/40" : "border-gray-200 bg-white",
                )}
                style={{ animationDelay: staggerDelay(index) }}
              >
                <div className="flex items-center gap-3">
                  <span className="text-xl leading-none">{getDeadlineIcon(deadline.deadline_type)}</span>
                  <div>
                ...
                <div className="flex flex-col items-end gap-1">
                  <p className="text-sm font-medium text-gray-800">{formatDate(deadline.due_date)}</p>
