...
import { annualReportsApi, type AnnualReportResponse } from "../../../api/annualReports.api";
...
const fetchDetail = async (reportId: number): Promise<AnnualReportDetail> => {
  const base = await annualReportsApi.getAnnualReport(reportId);
...
export const useAnnualReportDetail = (reportId: number | null) => {
  ...
  const qk = ["annual-reports", "detail", reportId] as const;
