...
export const FinancialFields = ({ register }: FinancialFieldsProps) => (
  <div>
    ...
    <div className="grid grid-cols-2 gap-3">
      <Input label="הכנסה ברוטו" type="number" min={0} endElement={currencySuffix} {...register("gross_income" as never)} />
      <Input label="הוצאות" type="number" min={0} endElement={currencySuffix} {...register("expenses" as never)} />
      <Input label="מקדמות ששולמו" type="number" min={0} endElement={currencySuffix} {...register("advances_paid" as never)} />
      <div>
    ...
    <div className="grid grid-cols-3 gap-2 text-info-700">
      ...
      <div>
        ...
        <span className={`font-mono ${preview.balance < 0 ? semanticMonoToneClasses.positive : semanticMonoToneClasses.negative}`}>
          ₪{formatWholeNumber(Math.abs(preview.balance))}{preview.balance < 0 ? " (החזר)" : ""}
        </span>
      </div>
    </div>
  </div>
