...
export const getDeadlineStatus = (report: AnnualReportFull): TimelineEventStatus => {
  ...
  const daysUntilDeadline = (
    new Date(report.filing_deadline).getTime() - Date.now()
  ) / 86_400_000;
    ...
    .flatMap((report) => {
      ...
      if (report.submitted_at) {
        ...
        const onTime =
          ...
          submittedTime <= new Date(report.filing_deadline).getTime();
        events.push({
...
export const getFilingStats = (reports: AnnualReportFull[]) => {
  ...
  return [
    { label: "הוגשו בזמן", count: submittedOnTime, pct: pct(submittedOnTime), color: "bg-positive-500" },
