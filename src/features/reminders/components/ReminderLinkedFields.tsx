    ...
    <>
      {(reminderType === "tax_deadline_approaching" || reminderType === "vat_filing") &&
        (clientTaxDeadlines.length > 0 ? (
          ...
          <Input type="number" min={1} label="מזהה מועד מס" error={e.tax_deadline_id?.message} {...register("tax_deadline_id")} />
        ))}
          ...
          >
            ...
            {clientCharges.map((charge) => (
              <option key={charge.id} value={String(charge.id)}>
                #{charge.id} - {getChargeTypeLabel(charge.charge_type)} ({getChargeStatusLabel(charge.status)})
              </option>
            ))}
          </Select>
        ) : (
          <Input type="number" min={1} label="מזהה חשבונית" error={e.charge_id?.message} {...register("charge_id")} />
        ))}
      ...
      {reminderType === "annual_report_deadline" &&
        (clientAnnualReports.length > 0 ? (
          ...
          <Input type="number" min={1} label="מזהה דוח שנתי" error={e.annual_report_id?.message} {...register("annual_report_id")} />
        ))}
      ...
      {reminderType === "advance_payment_due" &&
        (clientAdvancePayments.length > 0 ? (
          ...
          <Input type="number" min={1} label="מזהה מקדמה" error={e.advance_payment_id?.message} {...register("advance_payment_id")} />
        ))}
