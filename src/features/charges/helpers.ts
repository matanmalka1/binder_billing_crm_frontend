...
export const runChargeActionRequest = (
  chargeId: number,
  action: ChargeAction,
  reason?: string,
) => {
...
export const getChargeStatusStatDisplay = (
  stat: ChargeStatusStat,
  isAdvisor: boolean,
): string => (isAdvisor ? formatCompactCurrencyILS(stat.amount) : String(stat.count));
