...
export const dashboardApi = {
  ...
  getSummary: async (): Promise<DashboardSummaryResponse> => {
    const response = await api.get<DashboardSummaryResponse>(DASHBOARD_ENDPOINTS.dashboardSummary)
    return response.data
  },

  getAttention: async (): Promise<AttentionResponse> => {
    const response = await api.get<AttentionResponse>(DASHBOARD_ENDPOINTS.dashboardAttention)
    return response.data
  },
}
