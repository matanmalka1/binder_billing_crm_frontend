...
export const DashboardSectionHeader = ({
  ...
  <div className={cn("flex items-center justify-between gap-4", className)}>
    ...
    <div className="flex shrink-0 items-center gap-2">
      {count !== undefined && (
        <span className={cn("inline-flex h-7 min-w-7 items-center justify-center rounded-full px-2 text-xs font-bold tabular-nums", toneClasses[tone].badge)}>
          {count}
        </span>
      )}
      {action}
...
export const DashboardMetricCard = ({
  ...
  >
    <div className="flex items-start justify-between gap-2">
      ...
      <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", toneClasses[urgent ? "red" : tone].icon)}>
        <Icon className="h-4 w-4" />
      </span>
    </div>
    <div className="mt-3">
      <p className="text-2xl font-bold tabular-nums text-gray-950">{value}</p>
      <p className="mt-0.5 line-clamp-2 min-h-7 text-[11px] leading-3.5 text-gray-500">{description}</p>
    </div>
    {progress !== undefined && (
    ...
    <div className="mt-auto pt-2">
      {actionLabel && (
        <span className={cn("inline-flex items-center gap-1 text-xs font-semibold", urgent ? "text-red-700" : "text-slate-600")}>
          {actionLabel}
