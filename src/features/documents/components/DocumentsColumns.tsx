...
export const buildDocumentColumns = ({
  isAdvisor, replacingId, deletingId, onReplace, onDelete,
    ...
    base.push({
      ...
      render: (doc) => (
        <div className="flex gap-2 justify-end">
          <button onClick={() => onReplace(doc.id)} disabled={replacingId === doc.id}
            className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 disabled:opacity-50"
            title="החלף מסמך">
            <RefreshCw className="h-3.5 w-3.5" />
            {replacingId === doc.id ? "מחליף..." : "החלף"}
          </button>
          <button onClick={() => onDelete(doc.id)} disabled={deletingId === doc.id}
